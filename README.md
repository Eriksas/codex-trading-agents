...
需要自动生成候选解释和反方意见时，可显式启用受控 Claude 调用：`python main.py demo --agent claude --model haiku`。需本机客户端与有效认证，可能产生模型费用；默认仍不调用模型。使用前先看[受控调用说明](docs/controlled_agents.md)。
...
| 理解每天发到飞书的报告 | [飞书日报说明](docs/feishu_reports.md) |
...
当前 Eval 自动检查的是格式和结构化规则；文字是否歪曲事实仍需人工评审，没有已完成的真实模型通过率。统一诊断默认不调用模型，显式启用时通过受控接口；原日报可选 Claude 路径也已接入。Hermes 原路径仍主要靠规则约束，项目尚无统一操作系统沙箱。
